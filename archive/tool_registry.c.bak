...
// Forward declaration for MCP_ToolRegisterDynamic
int MCP_ToolRegisterDynamic(const char* json, size_t length);

// Wrapper for system.defineTool that converts MCP_ToolRegisterDynamic to MCP_ToolHandler format
...
int MCP_ToolRegistryInit(int maxTools) {
    ...
    // We need to create a wrapper handler for MCP_ToolRegisterDynamic since it returns int, not MCP_ToolResult
    MCP_ToolRegister("system.defineTool", defineToolWrapper, defineToolSchema);
...
// Use implementation from build_fix.h for these functions

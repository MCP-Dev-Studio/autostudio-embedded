...
// Helper function to check if character is op
static bool isOperatorChar(char c) {
    return c == '+' || c == '-' || c == '*' || c == '/' || c == '%' || 
           c == '=' || c == '!' || c == '<' || c == '>' || c == '&' || c == '|';
}

// Helper function to check if op is binary
static bool isBinaryOperator(MCP_RuleOperator op) {
    return op >= MCP_RULE_OP_ADD && op <= MCP_RULE_OP_LESS_EQUAL;
}

// Helper function to get op precedence
...
static void getNextToken(TokenizerContext* ctx) {
    ...
    if (isOperatorChar(input[pos])) {
        ...
        if (input[pos] == '=' && input[pos + 1] == '=') {
            ...
            ctx->current.value.opType = MCP_RULE_OP_EQUAL;
            ctx->position = pos + 2;
            return;
        } else if (input[pos] == '!' && input[pos + 1] == '=') {
            ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
            ctx->current.value.opType = MCP_RULE_OP_NOT_EQUAL;
            ctx->position = pos + 2;
            return;
        } else if (input[pos] == '>' && input[pos + 1] == '=') {
            ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
            ctx->current.value.opType = MCP_RULE_OP_GREATER_EQUAL;
            ctx->position = pos + 2;
            return;
        } else if (input[pos] == '<' && input[pos + 1] == '=') {
            ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
            ctx->current.value.opType = MCP_RULE_OP_LESS_EQUAL;
            ctx->position = pos + 2;
            return;
        } else if (input[pos] == '&' && input[pos + 1] == '&') {
            ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
            ctx->current.value.opType = MCP_RULE_OP_AND;
            ctx->position = pos + 2;
            return;
        } else if (input[pos] == '|' && input[pos + 1] == '|') {
            ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
            ctx->current.value.opType = MCP_RULE_OP_OR;
            ctx->position = pos + 2;
            return;
        }
        ...
        switch (input[pos]) {
            case '+':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_ADD;
                break;
                
            case '-':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_SUBTRACT;
                break;
                
            case '*':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_MULTIPLY;
                break;
                
            case '/':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_DIVIDE;
                break;
                
            case '%':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_MODULO;
                break;
                
            case '>':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_GREATER_THAN;
                break;
                
            case '<':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_LESS_THAN;
                break;
                
            case '!':
                ctx->current.type = MCP_TOKEN_TYPE_OPERATOR;
                ctx->current.value.opType = MCP_RULE_OP_NOT;
                break;
...
static MCP_RuleValue parseExpression(TokenizerContext* ctx) {
    ...
    while (ctx->current.type == MCP_TOKEN_TYPE_OPERATOR) {
        MCP_RuleOperator op = ctx->current.value.opType;

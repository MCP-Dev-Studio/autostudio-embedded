...
int MCP_SendToolResult(MCP_ServerTransport* transport, const char* sessionId, 
                     const char* operationId, bool success, const struct MCP_Content* content);

/**
 * @brief Broadcast an event to all connected clients
 */
int MCP_ServerBroadcastEvent(MCP_Server* server, const char* eventType, const struct MCP_Content* content);
#endif

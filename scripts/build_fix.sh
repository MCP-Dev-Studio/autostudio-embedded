...
# Fix device_info.c to remove duplicate includes
cat > core/device/device_info.c.new << 'EOF'
/**
 * @file device_info.c
...
# Append the rest of device_info.c after cleaning up the includes
tail -n +17 core/device/device_info.c >> core/device/device_info.c.new
mv core/device/device_info.c.new core/device/device_info.c
...
# Compile JSON mocks
echo "Compiling json_mock.c for tool_registry tests..."
gcc -c -Wall -Wextra -g -std=c99 json_mock.c -o build/json_mock.o

echo "Compiling json_device_info_mock.c for device_info tests..."
gcc -c -Wall -Wextra -g -std=c99 json_device_info_mock.c -o build/json_device_info_mock.o

# Build test_dynamic_tools with json_mock
...
# Build device info test with dedicated mock
